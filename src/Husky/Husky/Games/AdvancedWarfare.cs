    ...
    public class AdvancedWarfare
    {
        ...
        public static void ExportBSPData(ProcessReader reader, long assetPoolsAddress, long assetSizesAddress)
        {
            ...
            if (reader.ReadNullTerminatedString(reader.ReadInt64(reader.ReadInt64(assetPoolsAddress + 0x38) + 8)) == "fx")
            {
                ...
                var gfxMapAsset = reader.ReadStruct<GfxMapAW>(reader.ReadInt64(assetPoolsAddress + 0xF8));
        ...
        /// Reads Gfx Surfaces
        /// </summary>
        public static GfxSurfaceAW[] ReadGfxSufaces(ProcessReader reader, long address, int count)
        {
            // Preallocate array
            GfxSurfaceAW[] surfaces = new GfxSurfaceAW[count];

            // Loop number of indices we have
            for (int i = 0; i < count; i++)
                // Add it
                surfaces[i] = reader.ReadStruct<GfxSurfaceAW>(address + i * 40);
